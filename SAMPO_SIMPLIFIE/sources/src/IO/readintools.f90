...
INTERFACE GETINT
  ...
  MODULE PROCEDURE GETINT_IntProposal
END INTERFACE
...
END IF
if (numproc==0) WRITE(UNIT_StdOut,'(a3,a30,a3,a33,a3,a7,a3)')' | ',TRIM(Key),' | ', TRIM(GetStr),' | ',TRIM(DefMsg),' | '
END FUNCTION GETSTR
...
IF (CntStr.EQ.0) THEN
  IF (PRESENT(Proposal)) THEN
    CntStr=Proposal
  ELSE
    if (numproc == 0) WRITE(UNIT_StdOut,*) 'Inifile missing necessary keyword item : ',TRIM(TmpKey)
    STOP
  END IF
END IF
END FUNCTION CNTSTR
...
FUNCTION GETINT_IntProposal(Key,IntProposal)
!===================================================================================================================================
! Read integer named "key" from setup file and store in "GETINT". If keyword "Key" is not found in ini file,
! the default value "Proposal"=Str(IntProposal) is used for "GETINT" (error if "Proposal" not given).
! Ini file was read in before and is stored as list of character strings starting with "FirstString".
!===================================================================================================================================
! MODULES
! IMPLICIT VARIABLE HANDLING
...
! INPUT VARIABLES
CHARACTER(LEN=*),INTENT(IN)          :: Key      ! Search for this keyword in ini file
INTEGER,INTENT(IN)                   :: IntProposal ! Default values as integer
!-----------------------------------------------------------------------------------------------------------------------------------
! OUTPUT VARIABLES
INTEGER                              :: GetInt_IntProposal  ! Integer read from setup file or initialized with default value
!-----------------------------------------------------------------------------------------------------------------------------------
! LOCAL VARIABLES 
CHARACTER(LEN=5)                     :: Proposal

write(Proposal,'(I5.5)') IntProposal
GETINT_IntProposal = GETINT(Key,Proposal)
END FUNCTION GETINT_IntProposal
...
END IF
READ(HelpStr,*)GetInt
if (numproc == 0) WRITE(UNIT_StdOut,'(a3,a30,a3,i33,a3,a7,a3)')' | ',TRIM(Key),' | ', GetInt,' | ',TRIM(DefMsg),' | '
END FUNCTION GETINT
...
!-----------------------------------------------------------------------------------------------------------------------------------
! OUTPUT VARIABLES
REAL*8                               :: GetReal  ! Real read from setup file or initialized with default value
!-----------------------------------------------------------------------------------------------------------------------------------
! LOCAL VARIABLES 
CHARACTER(LEN=255)                   :: HelpStr
CHARACTER(LEN=8)                     :: DefMsg
...
if (numproc == 0) WRITE(UNIT_StdOut,'(a3,a30,a3,e33.5,a3,a7,a3)')' | ',TRIM(Key),' | ', GetReal,' | ',TRIM(DefMsg),' | '
...
if (numproc == 0) WRITE(UNIT_StdOut,'(a3,a30,a3,l33,a3,a7,a3)')' | ',TRIM(Key),' | ', GetLogical,' | ',TRIM(DefMsg),' | '
...
!-----------------------------------------------------------------------------------------------------------------------------------
! OUTPUT VARIABLES
REAL*8                    :: GetRealArray(nReals)        ! Real array read from setup file or initialized with default values
!-----------------------------------------------------------------------------------------------------------------------------------
! LOCAL VARIABLES 
CHARACTER(LEN=255)        :: HelpStr
CHARACTER(LEN=8)          :: DefMsg
...
IF (PRESENT(IniFile)) THEN
  File = TRIM(IniFile)
ELSE
  File=trim(path_input)//'input.ini'!CALL GETARG(1,File)
  !CALL GET_COMMAND_ARGUMENT(1,File)
END IF
if (numproc==0) WRITE(UNIT_StdOut,*)'| Reading from file "',TRIM(File),'":'
...
!IPWRITE(*,*)'Reading: ',Str,EOF
    IF (EOF.NE.IOSTAT_END) THEN
      ...
      ! Replace commas
      Str1%Str=Replace(Str1%Str,","," ",Every=.true.)
      ! Replace tubulator sign
      Str1%Str=Replace(Str1%Str,achar(9)," ",Every=.true.)
      ! Lower case
